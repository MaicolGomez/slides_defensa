...
\frame{
	...
	Main steps:
	\begin{itemize}
		\item Send information from \textcolor{blue}{omp declare target section} (new variables types and overcharged of operators);
		\item The algorithm obtains the pieces of information of the \textcolor{blue}{omp scan clause} (variable type, operator, size of vector);
		...
		the OpenCL driver to compile the kernels and then dispatch for execution.
	\end{itemize}	
}
...
#pragma OPENCL EXTENSION cl_khr_fp64 : enable //Header
...
int main(){
  ...
  Rearrange the elements from A, <@\textcolor{black}{using}@> lt, gt, ltScan <@\textcolor{black}{and}@> gtScan
}
\end{lstlisting}
}
